...
class Three
{
    ...
    public static function national_awards_count($e, $equipment, $dtstart, $dtend)
    {
        ...
        $sql = "SELECT COUNT(`a`.`id`)" .
            " FROM `award` AS `a`" .
            " INNER JOIN `_r_tag_achievements_award_award` AS `r`" .
            " ON `a`.`id`=`r`.`id2`" .
            " INNER JOIN `_r_equipment_award` AS `e`" .
            " ON `a`.`id`=`e`.`id2`" .
            " INNER JOIN `tag_achievements_award` AS `t`" .
            " ON `r`.`id1`=`t`.`id`" .
    ...
    public static function tea_patent_count($e, $equipment, $dtstart, $dtend)
    {
        ...
        $sql = "SELECT COUNT(`a`.`id`)" .
            " FROM `patent` AS `a`" .
            " INNER JOIN `_r_tag_achievements_patent_patent` AS `r`" .
            " ON `a`.`id`=`r`.`id2`" .
            " INNER JOIN `_r_patent_equipment` AS `e`" .
            " ON `a`.`id`=`e`.`id1`" .
            " INNER JOIN `tag_achievements_patent` AS `t`" .
            " ON `r`.`id1`=`t`.`id`" .
    ...
    public static function three_pubs_count($e, $equipment, $dtstart, $dtend)
    {
        ...
        $sql = "SELECT COUNT(`a`.`id`)" .
            ...
            " INNER JOIN `_r_tag_achievements_publication_publication` AS `r`" .
            " ON `a`.`id`=`r`.`id2`" .
            " INNER JOIN `_r_publication_equipment` AS `e`" .
            " ON `a`.`id`=`e`.`id1`" .
            " INNER JOIN `tag_achievements_publication` AS `t`" .
            " ON `r`.`id1`=`t`.`id`" .
            " WHERE `e`.`id2`=:equipment_id" .
            " AND `a`.`date` BETWEEN :dtstart AND :dtend" .
            " AND `t`.`name`='三大检索'";
