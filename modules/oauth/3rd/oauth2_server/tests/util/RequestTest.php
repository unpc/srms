...
class Request_test extends PHPUnit_Framework_TestCase
{
	...
	function test_get()
	{
		...
		$this->assertEquals(['foo' => 'bar'], $this->request->get());
	}

	function test_post()
	{
		$this->assertEquals('bar', $this->request->post('foo'));
		$this->assertEquals(['foo' => 'bar'], $this->request->post());
	}

	function test_file()
	{
		$this->assertEquals('bar', $this->request->file('foo'));
		$this->assertEquals(['foo' => 'bar'], $this->request->file());
	}

	function test_cookie()
	{
		$this->assertEquals('bar', $this->request->cookie('foo'));
		$this->assertEquals(['foo' => 'bar'], $this->request->cookie());
	}

	function test_server()
	{
		$this->assertEquals('foobar.com', $this->request->server('HTTP_HOST'));
		$this->assertEquals(['HTTP_HOST' => 'foobar.com'], $this->request->server());
	}

	function test_header()
	{
		$this->assertEquals('foobar.com', $this->request->header('Host'));
		$this->assertEquals(['Host' => 'foobar.com'], $this->request->header());
	}
