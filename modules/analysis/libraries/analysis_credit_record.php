...
class Analysis_Credit_Record
{

    //初始化remote数据库表结构
    static function init($e, $rest)
    {
        $fields = Config::get('schema.analysis_credit_record')['fields'];
        $columns = [];
        ...
        echo "   \e[32m 用户信用分增减明细表创建{$result} \e[0m\n";
    ...
    static function increment($e, $rest)
    {
        ...
        $data = [];
        //获取昨天增量数据
        $incrementData = Q("credit_record[ctime<={$endTime}][ctime>={$startTime}]");
        ...
        foreach ($chunks as $chunk) {
            ...
            echo "\e[32m 用户信用分增减明细表数据[{$ids}]推送完成 返回值[{$content}] \e[0m\n";
        }
    }
    ...
    {
        Log::add("[push] analysis_credit_record", 'analysis');
        $now = Date::get_day_start();
        $incrementData = Q("credit_record");
        ...
        Log::add("[push] credit_record push done", 'analysis');
    }
