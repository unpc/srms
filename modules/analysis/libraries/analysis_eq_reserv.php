...
class Analysis_Eq_Reserv
{

    //初始化remote数据库表结构
    static function init($e, $rest)
    {
        $fields = Config::get('schema.analysis_eq_reserv')['fields'];
        $columns = [];
        ...
        echo "   \e[32m 仪器预约记录表创建{$result} \e[0m\n";
    ...
    static function increment($e, $rest)
    {
        ...
        $data = [];
        //获取昨天及以后的增量数据
        $incrementData = Q("eq_reserv[dtend>={$startTime}]");
        foreach ($incrementData as $item) {
            $data[] = self::_format($item);
        }

        //获取修改数据
        $markData = Q("analysis_mark_desc[source_name=eq_reserv][ctime>={$startTime}]");
        $markId = [];
        ...
        if (!empty($markId)) {
            ...
            $markRecordData = Q("eq_reserv[id={$idStr}]");
    ...
    {
        Log::add("[push] eq_reserv pushing", 'analysis');
        $now = Date::get_day_start();
        $incrementData = Q("eq_reserv");
        ...
        Log::add("[push] eq_reserv push done", 'analysis');
    }
    ...
    private static function _format($item)
    {
        ...
        $row['user_group'] = $item->user->group->name;
        ...
        $row['dtstart'] = $item->dtstart ?: 0;
        $row['dtlength'] = $item->dtend - $item->dtstart;
        $row['dtlength'] = $row['dtlength'] >= 0 ? $row['dtlength'] : 0;
        $calCompoment = Q('cal_component#' . $item->component_id);
        $row['type'] = $calCompoment->type;
        $row['description'] = $calCompoment->description;
        // $row['mtime'] = $item->mtime; // 这一项需要和init方法中的注册字段同步，暂时先取消
