...
class Analysis_Project_Publication
{
    ...
    static function init($e, $rest)
    {
        ...
        $fields = Config::get('schema.analysis_project_publication')['fields'];
        $columns = [];
        ...
        echo "   \e[32m 项目论文关联表创建{$result} \e[0m\n";
    }
    ...
    static function increment($e, $rest)
    {
        if (!Module::is_installed('achievements')) return;
        self::_pour();
        $projectPubliation = Q("analysis_project_publication");
        ...
        foreach ($chunks as $chunk) {
            ...
            echo "\e[32m 项目成果论文表[{$ids}]推送完成 返回值[{$content}] \e[0m\n";
        }
    }
    ...
    {
        Log::add("[push] project_publication pushing", 'analysis');
        if (!Module::is_installed('achievements')) return;
        self::_pour();
        ...
        Log::add("[push] project_publication push done", 'analysis');
    }
    ...
    private static function _pour(){
        $db = Database::factory();
        $s = 'TRUNCATE TABLE `analysis_project_publication`';
        $r = $db->query($s);
        ...
        FROM `lab_project` `lp`
        INNER JOIN
        `_r_publication_lab_project` `rplp` ON `lp`.`id` = `rplp`.`id2`
        INNER JOIN
        `publication` `p` ON `p`.`id` = `rplp`.`id1`
        LEFT JOIN
        `_r_publication_equipment` `rpe` ON `rpe`.`id1` = `p`.`id`
        LEFT JOIN
        `equipment` `e` ON `e`.`id` = `rpe`.`id2`
        LEFT JOIN
        `_r_tag_publication` `tp` ON `tp`.`id2` = `p`.`id`
        LEFT JOIN
        ...
        foreach ($publications as $p) {
            $analysisProject = O('analysis_project_publication');
            $analysisProject->title = $p->title;
            //获取当前成果作者
            $authors = Q("ac_author[achievement_name=publication][achievement_id=$p->pid]");
            $author_list = [];
            ...
            $analysisProject->journal = $p->journal;
            $analysisProject->date = $p->date;
            $analysisProject->volume = $p->volume;
            $analysisProject->issue = $p->issue;
