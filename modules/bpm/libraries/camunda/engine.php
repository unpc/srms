...
class Engine implements \Gini\BPM\Driver\Engine {
    ...
    public function __construct($config) {
        ...
        $response = $this->http->post("{$this->root}/admin/auth/user/default/login/cockpit", [
            'form_params' => [
        ...
        $rdata = json_decode($content, true);
        $this->userId = $rdata['userId'];
        $this->authorizedApps = $data['authorizedApps'];
    }

    public function post($path, array $data=[]) {
        $response = $this->http->post("{$this->root}/engine/engine/{$this->engine}/$path", [
            'body' => json_encode((object)$data, JSON_UNESCAPED_UNICODE),
            'headers' => [
    ...
    public function get($path, array $data=[]) {
        $response = $this->http->get("{$this->root}/engine/engine/{$this->engine}/$path", [
            'query' => $data
        ]);
    ...
    public function delete($path, array $data=[]) {
        $response = $this->http->delete("{$this->root}/engine/engine/{$this->engine}/$path", [
            'body' => json_encode((object)$data, JSON_UNESCAPED_UNICODE),
            'headers' => [
                'Content-Type' => 'application/json'
            ]
    ...
    public function put($path, array $data=[]) {
        $response = $this->http->put("{$this->root}/engine/engine/{$this->engine}/$path", [
            'body' => json_encode((object)$data, JSON_UNESCAPED_UNICODE),
            'headers' => [
                'Content-Type' => 'application/json'
            ]
    ...
    public function deploy($name, $files) {
        ...
        $response = $this->http->post("engine/engine/$engine/deployment/create", [
            'form_params' => $data,
            'headers' => [
    ...
    public function searchTasks(array $criteria) {
         ...
         * type: string
         */
        if (isset($criteria['task'])) {
            $query['taskDefinitionKey'] = $criteria['task'];
        }
