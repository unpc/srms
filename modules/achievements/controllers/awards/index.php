...
class Awards_Index_Controller extends Base_Controller
{
    ...
    public function index()
    {
        ...
        $tag      = O('tag_achievements_award', $form['tag_id']);
        ...
        $form_token = Session::temp_token('award_list_', 300);

        $_SESSION[$form_token] = ['selector' => $selector];

        $awards = Q($selector);

        $pagination = Lab::pagination($awards, (int) $form['st'], 15);

        $content = V('awards/list', [
            'awards'     => $awards,
