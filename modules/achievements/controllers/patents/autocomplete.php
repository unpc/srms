...
class Patents_Autocomplete_Controller extends AJAX_Controller {
	...
	function tags() {
		...
		if ($s) {
			$s = Q::quote($s);
			$all_tags = Q("tag_achievements_patent[root={$root}][name*={$s}]");
			...
			$all_tags = Q("tag_achievements_patent[root={$root}]");
			$tags = $all_tags->limit($start, $n);
