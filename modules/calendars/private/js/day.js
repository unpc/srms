(function($) {
    ...
    var makeStickyHeader = function() {
        ...
        function tracker() {
            ...
            var vOffset =  $body.scrollTop() + 48*2 - $originalHeader.offset().top;
            if (vOffset > 0  && vLength > 40) {
                if (supportFix) {
                    $header.show().css({left: $originalHeader.offset().left - $body.scrollLeft() - 0.5, top:48*2, visibility:"visible"});
                    $('.display_with_calendar_week_header_left').css({left: $originalHeader.offset().left - $body.scrollLeft() + 1, top: 50, visibility:"visible", position: "fixed", zIndex: 500});
                    var o = document.getElementById("layout_body");
                    var w = o.clientWidth || o.offsetWidth;
                    $('.display_with_calendar_week_header_right').css({left: $originalHeader.offset().left - $body.scrollLeft() + 1 + w  - 270 - $('.display_with_calendar_week_header_right').find('#toggle_button').width() + 4,
                        top: 50, width: $('.display_with_calendar_week_header_right').find('#toggle_button').width(), visibility:"visible", position: "fixed", zIndex: 500});
                }
                else {
    ...
    Day.prototype.getComponent = function () {
        ...
        Component.prototype.getBlock = function () {
            ...
            //移出
            function () {
                $block.css({zIndex:$block.data('Q.Calendar.Day.Component.zIndex')});
            });
            */
            return $block;
        };
        ...
        Component.prototype.render = function (type) {
            ...
            var pos1 = parent.getHourXYFromDateTime(dtStart, this.equipment_id);
            var pos2 = parent.getHourXYFromDateTime(dtEnd, this.equipment_id);
            ...
            for (i = 0; i < $blocks.length; i++) {
                ...
                // w = rect.width - x;
                w = parent.dayX[pos2.hour] + Math.floor((dtEnd % 3600) * (rect.width / 86400)) - parent.dayX[hour] - indent;
    ...
    Day.prototype.getHourXYFromDateTime = function (dt, equipment_id) {
        var rect = this.hourGridRect;
        var dtOffset = dt - this.dtStart;
        ...
        var $equipment = $('.tick_' + equipment_id);
