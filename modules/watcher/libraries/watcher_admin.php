...
class Watcher_Admin {
	...
	public static function setup()
	{
		if ( L('ME')->access('管理所有内容') ) {
			Event::bind('admin.index.tab', 'Watcher_Admin::primary_tab');
		}
	}
	...
	{
		if ( L('ME')->access('管理所有内容') ) {
			Event::bind('admin.index.content', 'Watcher_Admin::primary_content', 0, 'watcher');

			$tabs->add_tab('watcher', [
				'url' => URI::url('admin/watcher'),
				'title' => I18N::T('watcher', '多媒体设置'),
			]);
		}
	}
	...
	public static function equipment_links($e, $equipment, $links, $mode) 
	{
		if (L('ME')->access('管理所有内容')) {
			if ($mode == 'watcher' || $mode == 'view') {
				$links['watcher'] = [
					'url' => '#',
					'text'  => I18N::T('watcher', '查看验证码'),
					'tip' => I18N::T('watcher', '查看验证码'),
					'extra' => 'class="button button_view" q-src="'.URI::url('!watcher').'" q-object="watcher_view" q-event="click" q-static="'.H(['id' => $equipment->id]).'"'
				];
