...
class API_Eq_Comment extends API_Common
{
    public function getCommentSetting($equipment_id = '', $type = 'incharge')
    {
        $this->_ready();
        //$eq_;
        $commentColuns = Config::get('comment.comment_columns.' . $type);
        if (!$commentColuns || empty($commentColuns)) {
            return [];
        }
        return $commentColuns;
    }
    ...
    public function update($data)
    {
        ...
        $record = O('eq_record', $data['record']);
        if (!$record->id)
