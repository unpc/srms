...
class CLI_Export_Apply_Record
{
    ...
    static function export() {
        ...
        foreach ($applys as $apply) {
            ...
            if (array_key_exists('project_name', $valid_columns)) {
                $data[] = $apply->project->name ?: '';
            }
            if (array_key_exists('equipment', $valid_columns)) {
                $data[] = $apply->equipment->name ?: '';
            }
            ...
            if (array_key_exists('dtrequest', $valid_columns)) {
                $data[] = $apply->apply->dtrequest ? date('Y-m-d H:i:s', $apply->apply->dtrequest) : '-';
            }
            if (array_key_exists('service_time_length', $valid_columns)) {
                $data[] = round($apply->dtlength / 3600, 2);
            }
            if (array_key_exists('status', $valid_columns)) {
                $data[] = Service_Apply_Record_Model::$status_labels[$apply->status] ?? '';
            }
            if (array_key_exists('dtend', $valid_columns)) {
                $data[] = $apply->dtend ? date('Y-m-d H:i:s', $apply->dtend) : '-';
            }
            //定制的输出项
            $data_extra = Event::trigger('service_apply_record.export_list_csv', $apply, $data, $valid_columns);
