...
class Rrule_AJAX_Controller extends AJAX_Controller {
	...
	function index_time_counts_rtype_change() {
		$form = Form::filter(Input::form());
		...
		$switch = $form['time_counts_rtype'][$flexform_index];

		switch ($switch) {
			case WT_RRule::RRULE_DAILY:   //每天
				break;
			case WT_RRule::RRULE_WEEKLY:  //每周
				$path = 'eq_time_counts:widgets/rule_date/item/wt_week_day';
                $view = V($path, ['flexform_index'=>$flexform_index,'week_day'=>range(0,6)]);
				break;
			case WT_RRule::RRULE_MONTHLY:  //每月
				$path = 'eq_time_counts:widgets/rule_date/monthly/wt_month';
                $view = V($path, ['flexform_index'=>$flexform_index,'month_day'=>range(1,31)]);
				break;
			default:
				$path = '';
                $view = '';
