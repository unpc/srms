<?php
/*
 * @file free_calculate_fee.php
 * @author Xiaopei Li <xiaopei.li@geneegroup.com>
 * @date 2011-07-26
 *
 * @brief 设定仪器计费免费使用进行计费的测试用例
 * @usage SITE_ID=cf LAB_ID=ut Q_ROOT_PATH=~/lims2/ php test.php ../tests/50-eq_charge/free_calculate_fee
 */
if (!Module::is_installed('eq_charge')) return true;
require_once ('charge_test.php');
class free_charge_test_2 extends charge_test
{
	...
	function test_no_reserv() {
		...
		$record = self::mk_record_and_assert_auto_amount_before_save(0, $this->equipment, $this->user, '2011/04/01 19:00', '2011/04/01 20:00'); /* 使用 1 小时 */
	...
	function test_on_time() {
		...
		$record = self::mk_record_and_assert_auto_amount_before_save(0, $equipment, $user, '2011/04/01 19:00', '2011/04/01 20:00');
