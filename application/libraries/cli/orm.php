...
class CLI_Orm {
			...
			foreach ($tables as $table) {
				if ($table[0] == '_') {
					...
					if ($prefix == 'r_') {
						...
						if ($count > 2) {
							for ($i=1; $i<$count; $i++) {
								self::repair_relation(implode('_', array_slice($units, 0, $i)), implode('_', array_slice($units, $i)));
							}	
						}
						elseif ($count == 2) {
							self::repair_relation($units[0], $units[1]);
						}
					}
					elseif ($prefix == 'p_') {
						self::repair_properties(substr($table, 3));
					}
				}
				else {
					self::repair_object($table);
				}
			}
	...
	//设置数据库引擎,切换所有表的引擎
	static function dbengine($engine=null) {
		$valid_engines = [
			'MYISAM', 'InnoDB'
		];
	...
	static private function repair_relation($oname1, $oname2) {
	...
	static private function repair_properties($oname) {
	...
	static private function repair_object($oname) {
		$oname = ORM_Model::real_name($oname);
		if (!$oname) return;
