(function($){
	...
	Q['ajaxContent'] = {
		replace: function(data) { this.empty(); this.replaceWith(data); },
					...
					$(':input:not(:submit, :image, :disabled)', $el).each(function(){
						if(this.name) {
							...
							if($this.is(':radio')) {
                                ...
                                /**
                                 * 这里本应为null
                                 * 在更新之前的低版本jquery json序列化中，php端接收的值为: 'null'
                                 * 在升级jquery至1.12.4之后，php端接收为: '' 
                                 * 所以在这里赋值为'null' php端所有的导出表格处都无需更新
                                 */
	...
	$('div[src] a[href]:not(.view, [q-object], .prevent_default, .group_prevent_default a)').livequery('click', function(){
