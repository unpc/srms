...
abstract class _Form extends ArrayIterator {
	...
	static function dropdown($name, $options, $selected=NULL, $extra=NULL) {
		...
		{
			if (strpos($extra, 'selectpicker') === false)
			{
                $selected_text = in_array($key, $selected) ? $val : $selected_text;
				if(is_array($val)&&!$selected_text){
					$selected_text=$val[$selected[0]];
				}
			}
			else{
                $selected_text[] = in_array($key, $selected) ? $val : '';
            }
        ...
        preg_match('/\bclass=\"(.+?)\"/', $extra, $class);
        if (strpos($extra, 'selectpicker') === false) {
            $form .= '<div class="dropdown_container '.$class[1].'">'.'<div class="dropdown_text '.'">'.$selected_text.'</div><span class="icon-drop_down"></span></div>';
        } elseif (strpos($extra, 'selectpicker') !== false) {
            if (!empty($selected_text)){
                $selected_text = array_filter($selected_text);
            }else{
                $selected_text = [];
            }
            $selected_container = '';
            foreach ($selected_text as $v) {
                $selected_container .= '<div class="picker_item_value">'.$v.'</div>';
            }
            $form .= '<div class="'.$class[1].'">'.$selected_container.'<div class="check_more float_right"><span class="icon-drop_down"></span></div></div>';
        }
	...
	static function checkbox($name, $checked=FALSE, $label=NULL, $extra=NULL, $extra_label=NULL) {
        ...
        /**
         * system/private/js/q/ajax.js line:278 与该行相呼应
         * 不应该这样写，但是导出表格那边判断== 'null'会有问题
         */
