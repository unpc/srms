...
class PHP_Encoder {
	...
	function __construct($output_file, $escape = FALSE) {
	...
	function add($path) {
		try {
			...
			file_exists($this->output_file) and @unlink($this->output_file);
			$this->phar = new Phar($this->output_file, 0, basename($this->output_file));
		}
	...
	function encode_file($path) {
		...
		if (preg_match('/\.(php|phtml)$/', $path, $matches)) {
            ...
            if ($this->escape) {
                //$GLOBALS['preload']['element'] 替换
                foreach((array) $GLOBALS['preload'] as $key=>$value) {
                    if (is_bool($value)) $value = (int) $value;
                    $content = preg_replace(strtr('/\$GLOBALS\[\'preload\'\]\[\'%key\'\]/', ['%key'=> $key]), $value, $content);
                }
            }
